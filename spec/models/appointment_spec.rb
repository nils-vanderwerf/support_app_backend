...
RSpec.describe Appointment, type: :model do
  ...
  describe 'overlap validation' do
    it 'is invalid when the support worker has an overlapping appointment' do
      Appointment.create!(client: client, support_worker: support_worker, date: '2026-05-01 10:00', duration: 60)
      overlapping = valid_appointment(date: '2026-05-01 10:30', duration: 60)
    ...
    it 'is valid when the support worker has a non-overlapping appointment' do
      Appointment.create!(client: client, support_worker: support_worker, date: '2026-05-01 10:00', duration: 60)
      non_overlapping = valid_appointment(date: '2026-05-01 11:00', duration: 60)
      expect(non_overlapping).to be_valid
    end

    it 'allows a client to have overlapping appointments (warn only on frontend)' do
      other_sw_user = User.create!(email: 'sw2@test.com', password: 'password123')
      other_sw = SupportWorker.create!(user: other_sw_user, first_name: 'Alice', last_name: 'Jones', email: 'sw2@test.com', phone: '0411111111', age: 30, location: 'Sydney')
      Appointment.create!(client: client, support_worker: support_worker, date: '2026-05-01 10:00', duration: 60)
      overlapping = Appointment.new(client: client, support_worker: other_sw, date: '2026-05-01 10:30', duration: 60)
