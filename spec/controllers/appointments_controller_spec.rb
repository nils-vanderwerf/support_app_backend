...
RSpec.describe "AppointmentsController", type: :request do
    ...
    let(:appointment) { Appointment.create(deleted_at: nil, client_id: client.id, support_worker_id: support_worker.id ) }
    ...
    describe "DESTROY /api/appointment" do
      ...
      context 'when an appointment is deleted' do
        it 'soft deletes it, leaving a datetime value on the deleted_at column on the record' do
