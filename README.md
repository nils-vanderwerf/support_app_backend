...
- `Client::PUBLIC_ATTRIBUTES` + `Client#as_json_for(full:)` decide, server-side, exactly which attributes go out in the JSON response — a worker without an approved appointment gets name/age/location/bio/health_conditions/`support_needs` (a deliberately curated summary for judging fit); the raw medical record (medication, allergies) and contact info (phone, email) only serialize once `SupportWorker#approved_appointment_with?(client)` is true.
